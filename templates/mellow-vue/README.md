...
The `package.json` overrides keep `qs` on the patched 6.x line and `body-parser` on the patched 1.x line while upstream packages still pin older releases. The scoped `meow` override uses the patched CommonJS `trim-newlines` 3.x API for the development hook's CLI dependency.
